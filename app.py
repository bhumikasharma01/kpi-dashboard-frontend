...
def admin_dashboard():
    ...
    if option == "User Dashboard":
        ...
        if submit_button:
            try:
                url = f"http://127.0.0.1:8000/upload/view?month={selected_month}&year={selected_year}"
    ...
    # ==== ADMIN SECTIONS ====
        ...
        with col3:
            if st.button("👁️ View All Users"):
                try:
                    users_resp = requests.get("http://127.0.0.1:8000/auth/users", headers=headers)
                ...
                if submit_btn:
                    if not new_username or not new_password:
                        ...
                        try:
                            ...
                            response = requests.post("http://127.0.0.1:8000/auth/create-user", json=payload, headers=headers)
                            if response.status_code == 200:
                        ...
                        if st.button("🗑️ Confirm Delete") and confirm:
                            try:
                                del_resp = requests.delete(
                                    f"http://127.0.0.1:8000/auth/delete-user",
    ...
    # ---------------------
        with tab1:
            ...
            with st.form("upload_form"):
                ...
                if submitted and uploaded_file:
                    ...
                    check_resp = requests.get("http://127.0.0.1:8000/upload/reports", headers=headers, params=params)
                    ...
                    upload_resp = requests.post("http://127.0.0.1:8000/upload/reports", data=data, files=files, headers=headers)
        ...
        with tab2:
            ...
            if submitted:
                ...
                try:
                    headers = {"Authorization": f"Bearer {st.session_state.token}"}
                    response = requests.get("http://127.0.0.1:8000/upload/reports", headers=headers, params=params)
                    ...
                    if reports:
                        for rep in reports:
                            with st.expander(f"{rep['filename']}"):
                                ...
                                download_url = f"http://127.0.0.1:8000/upload/reports/download?month={rep['month']}&year={rep['year']}"
                    ...
                    with st.expander(f"{rep['filename']}"):
                        ...
                        with st.form(key=f"delete_form_{idx}"):
                            ...
                            if delete_btn:
                                try:
                                    del_resp = requests.delete(
                                        "http://127.0.0.1:8000/upload/reports/delete",
                                        headers=headers,
        ...
        if st.button("Generate PDF Report"):
            with st.spinner("Generating PDF..."):
                try:
                    API_URL = "http://127.0.0.1:8000/upload/generate_pdf"
...
def fetch_dashboard_data(token):
    headers = {"Authorization": f"Bearer {st.session_state.token}"}
    try:
        response = requests.get("http://127.0.0.1:8000/kpi/dashboard", headers=headers)
